...
![State](https://img.shields.io/badge/State-Completed-brightgreen?style=for-the-badge)
...
**[EN]** The main engine of the terminal is fully built and capable of executing system commands and internal functions. The minishell features:
...
* **Built-in Commands:** Intercepting specific commands before execution, such as using native `chdir()` to make `cd` functional within the parent process.

**[ES]** El motor principal de la terminal está completamente construido y es capaz de ejecutar comandos del sistema y funciones internas. La minishell cuenta con:
...
* **Comandos Integrados (Built-ins):** Intercepción de comandos específicos antes de su ejecución,
...
- [X] Develop built-in commands like `cd` using `chdir()`.
...
- [X] Desarrollar comandos integrados (built-ins) como `cd` utilizando `chdir()`.
